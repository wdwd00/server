...
class UserTokenAuthtication(BaseAuthentication):
    def authenticate(self, request):
        userToken = request.META.get("HTTP_ADMINTOKEN")
        # print("检查adminToken==>" + adminToken)

        users = User.objects.filter(admin_token=userToken)
        """
        判定条件：
             查到了该帐号
        """
        print(userToken,users)
        if  len(users) == 0:
            raise exceptions.AuthenticationFailed("NOT_LOGIN")
        else:
            print('userToken验证通过')
